...
const addCommentService = (idNews, comment, userId) => { 
    ...
    return News.findOneAndUpdate({_id : idNews}, {$pull: {comments : { idComment, userId, comment, createdAt : new Date()}}})
};

const deleteCommentService = (idNews, idComment, userId) => News.findOneAndUpdate({_id : idNews}, { $pull: { comments : {idComment, userId }}})

export { createService, findAllService, countNewsService, topNewsService, findByIdService, searchByTitleService, byUserService, updateService, eraseService, likeNewsService, deletelikeNewsService, addCommentService, deleteCommentService }